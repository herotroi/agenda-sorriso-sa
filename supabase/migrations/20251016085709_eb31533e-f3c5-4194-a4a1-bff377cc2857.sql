...
-- (Versão com função IMMUTABLE e hash)
-- ========================================

-- 1. Criar função IMMUTABLE para extrair segundo
CREATE OR REPLACE FUNCTION public.extract_second_immutable(ts timestamptz)
RETURNS timestamptz
LANGUAGE sql
IMMUTABLE
AS $$
...
-- 2. Criar índices únicos usando a função IMMUTABLE
CREATE UNIQUE INDEX IF NOT EXISTS uniq_notifications_second
  ON public.notifications(user_id, type, message, extract_second_immutable(created_at));

CREATE UNIQUE INDEX IF NOT EXISTS uniq_notifications_second_appt
  ON public.notifications(user_id, type, COALESCE(appointment_id::text, ''), message, extract_second_immutable(created_at));

-- 3. Limpar duplicatas históricas existentes
WITH ranked AS (
  SELECT
    id,
    ROW_NUMBER() OVER (
      PARTITION BY user_id, type, message, extract_second_immutable(created_at), COALESCE(appointment_id::text, '')
      ORDER BY created_at
    ) AS rn
...
-- 4. Remover triggers duplicadas programaticamente (appointments)
DO $$
DECLARE t record;
BEGIN
  FOR t IN
...
-- 5. Remover triggers duplicadas (patients)
DO $$
DECLARE t record;
BEGIN
  FOR t IN
...
-- 6. Remover triggers duplicadas (procedures)
DO $$
DECLARE t record;
BEGIN
  FOR t IN
...
-- 7. Remover triggers duplicadas (patient_records)
DO $$
DECLARE t record;
BEGIN
  FOR t IN
...
-- 8. Remover triggers duplicadas (profiles)
DO $$
DECLARE t record;
BEGIN
  FOR t IN
...
-- 9. Atualizar função de notificação de appointments com ON CONFLICT
...
BEGIN
  ...
  -- Inserção atômica com ON CONFLICT (blindagem contra corrida)
...
-- 10. Atualizar função de notificação de patients com ON CONFLICT
...
BEGIN
  ...
  -- Inserção atômica com ON CONFLICT
...
-- 11. Atualizar função de notificação de procedures com ON CONFLICT
...
-- 12. Atualizar função de notificação de patient_records com ON CONFLICT
...
-- 13. Atualizar função de notificação de profiles com ON CONFLICT
...
-- 14. Recriar triggers oficiais para appointments
...
-- 15. Recriar triggers oficiais para patients
...
-- 16. Recriar triggers oficiais para procedures
...
-- 17. Recriar triggers oficiais para patient_records
...
-- 18. Recriar trigger oficial para profiles (apenas UPDATE)
